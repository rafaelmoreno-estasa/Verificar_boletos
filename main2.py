...
def main():
    ...
    for id_sl in tqdm(tuplas_codigo_id, desc="Verificando as cobranças de cada condomino"):
        ...
        if len(unidades_sem_retorno) > 1 and len(unidades_sem_retorno) < len(unidades):
            lista_de_cobranca_para_email_cac_e_operacional.append((id_sl[0], len(unidades_sem_retorno)))
    ...
    if not (lista_de_cobranca_para_email_cac or lista_de_cobranca_para_email_operacional or lista_de_cobranca_para_email_cac_e_operacional):
